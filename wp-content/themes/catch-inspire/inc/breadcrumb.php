...
if ( ! function_exists( 'catch_inspire_breadcrumb' ) ) :
	function catch_inspire_breadcrumb() {

		/* === OPTIONS === */
		$text['home']     = esc_html__( 'Home', 'catch-inspire' ); // text for the 'Home' link
		/* translators: 1: before text/html, 2: after text/html. */
		$text['category'] = esc_html__( '%1$s Archive for %2$s', 'catch-inspire' ); // text for a category page
		/* translators: 1: before text/html, 2: after text/html. */
		$text['search']   = esc_html__( '%1$sSearch results for: %2$s', 'catch-inspire' ); // text for a search results page
		/* translators: 1: before text/html, 2: after text/html. */
		$text['tag']      = esc_html__( '%1$sPosts tagged %2$s', 'catch-inspire' ); // text for a tag page
		/* translators: 1: before text/html, 2: after text/html. */
		$text['author']   = esc_html__( '%1$sView all posts by %2$s', 'catch-inspire' ); // text for an author page
		$text['404']      = esc_html__( 'Error 404', 'catch-inspire' ); // text for the 404 page
		...
		if ( is_front_page() ) {
			...
			if ( is_home() ) {
				...
				echo sprintf( $link, esc_url( get_year_link( get_the_time( __( 'Y', 'catch-inspire' ) ) ) ), esc_html( get_the_time( __( 'Y', 'catch-inspire' ) ) ) ) ; // WPCS: XSS OK.


				echo sprintf( $link, esc_url( get_month_link( get_the_time( __( 'Y', 'catch-inspire' ) ), get_the_time( __( 'n', 'catch-inspire' ) ) ) ), esc_html( get_the_time( __( 'F', 'catch-inspire' ) ) ) ); // WPCS: XSS OK.

				echo $before . esc_html( get_the_time( __( 'd', 'catch-inspire' ) ) ) . $after; // WPCS: XSS OK.

			} elseif ( is_month() ) {
				echo sprintf( $link, esc_url( get_year_link( get_the_time( __( 'Y', 'catch-inspire' ) ) ) ), esc_html( get_the_time( __( 'Y', 'catch-inspire' ) ) ) ) ; // WPCS: XSS OK.

				echo $before . esc_html( get_the_time( __( 'F', 'catch-inspire' ) ) ) . $after; // WPCS: XSS OK.


			} elseif ( is_year() ) {
				echo $before . esc_html( get_the_time( __( 'Y', 'catch-inspire' ) ) ) . $after; // WPCS: XSS OK.
			...
			if ( get_query_var( 'paged' ) ) {
				...
				echo sprintf( esc_html__( 'Page %s', 'catch-inspire' ), absint( max( $paged, $page ) ) );
