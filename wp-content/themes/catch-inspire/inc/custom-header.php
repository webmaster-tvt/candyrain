...
if ( ! function_exists( 'catch_inspire_featured_image' ) ) :
	/**
	 * Template for Featured Header Image from theme options
	 *
	 * To override this in a child theme
	 * simply create your own catch_inspire_featured_image(), and that function will be used instead.
	 *
	 * @since Catch Base 1.0
	 */
	function catch_inspire_featured_image() {
				...
				<div class="header-media">
					<div class="wrapper">
						<div class="post-thumbnail archive-header-image">
							<?php echo wp_get_attachment_image( (int) $jetpack_options['featured-image'], 'catch-inspire-slider' ); ?>
						</div>
					</div><!-- .wrapper -->
				</div><!-- .header-media -->
			<?php endif;
		...
		elseif ( is_post_type_archive( 'jetpack-portfolio' ) || is_post_type_archive( 'featured-content' ) || is_post_type_archive( 'ect-service' ) ) :
			...
			if ( '' !== $featured_image ) : ?>
				<div class="header-media">
					<div class="wrapper">
						<div class="post-thumbnail archive-header-image">
							<?php echo wp_get_attachment_image( (int) $featured_image, 'catch-inspire-slider' ); ?>
						</div>
					</div><!-- .wrapper -->
				</div><!-- .header-media -->
	...
	} // catch_inspire_featured_image
endif;

if ( ! function_exists( 'catch_inspire_featured_page_post_image' ) ) :
	/**
	 * Template for Featured Header Image from Post and Page
	 *
	 * To override this in a child theme
	 * simply create your own catch_inspire_featured_imaage_pagepost(), and that function will be used instead.
	 *
	 * @since Catch Inspire 1.0
	 */
	function catch_inspire_featured_page_post_image() {
			...
			<div class="wrapper">
				<div class="post-thumbnail singular-header-image">
					...
					if ( is_home() && $blog_id = get_option( 'page_for_posts' ) ) {
					    echo get_the_post_thumbnail( $blog_id, 'catch-inspire-slider' );
					} else {
						the_post_thumbnail( 'catch-inspire-slider' );
					}
					?>
				</div><!-- .post-thumbnail -->
			</div><!-- .wrapper -->
		...
		<?php
	} // catch_inspire_featured_page_post_image
endif;


if ( ! function_exists( 'catch_inspire_featured_overall_image' ) ) :
	/**
	 * Template for Featured Header Image from theme options
	 *
	 * To override this in a child theme
	 * simply create your own catch_inspire_featured_pagepost_image(), and that function will be used instead.
	 *
	 * @since Catch Inspire 1.0
	 */
	function catch_inspire_featured_overall_image() {
		global $post, $wp_query;
		$enable = get_theme_mod( 'catch_inspire_header_media_option', 'homepage' );

		// Check Enable/Disable header image in Page/Post Meta box
		if ( is_page() || is_single() ) {
			//Individual Page/Post Image Setting
			$individual_featured_image = get_post_meta( $post->ID, 'catch-inspire-header-image', true );
	...
	} // catch_inspire_featured_overall_image
endif;
